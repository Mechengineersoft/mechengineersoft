      ...
      <div className="max-w-7xl mx-auto px-6 py-16">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
          ...
          <Link href="/" className="flex items-center gap-3 group" aria-label="MechEngineerSoft — Home">
            <MESLogo size={32} />
            <span className="font-bold text-sm text-foreground hidden sm:block">{settings.companyName}</span>
          </Link>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="text-right ml-2"><span className="block text-sm text-muted-foreground">© 2026 {settings.companyName}</span>{settings.businessEmail && <span className="block text-xs text-muted-foreground mt-1">{settings.businessEmail}</span>}</div>
          </div>
        </div>
      </div>
    </footer>

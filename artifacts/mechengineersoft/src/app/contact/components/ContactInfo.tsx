    ...
    <div ref={sectionRef} className="flex flex-col gap-6">
      ...
      >
        ...
        <div className="space-y-5">
          {[
            { icon: 'EnvelopeIcon' as const, label: 'Business Email', value: settings.businessEmail, href: settings.businessEmail ? `mailto:${settings.businessEmail}` : null },
            { icon: 'PhoneIcon' as const, label: 'Phone Number', value: settings.phone, href: settings.phone ? `tel:${settings.phone}` : null },
            { icon: 'MapPinIcon' as const, label: 'Office Address', value: settings.address, href: null },
          ].map((item) => (
            <div key={item.label} className="flex items-start gap-4">
              ...
              <div>
                ...
                {item.href ? <a href={item.href} className="text-sm text-foreground font-semibold hover:text-accent">{item.value || 'Add contact details in admin'}</a> : <div className="text-sm text-foreground font-semibold">{item.value || 'Add contact details in admin'}</div>}
              </div>
            </div>
          ))}
        </div>

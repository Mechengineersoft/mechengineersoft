...
import { usePortfolioProjects } from '../data/usePortfolioProjects';
            ...
            <div className="col-span-2 rounded-xl p-4" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)' }}>
              ...
              <div className="flex justify-around mt-1">
                  {['J', 'F', 'M', 'A', 'M', 'J', 'J', 'A', 'S', 'O', 'N', 'D'].map((m, index) => (
                    <div key={`${m}-${index}`} className="text-[8px] text-white/20">{m}</div>
                ))}
              </div>
            </div>
...
export default function ProjectDetail() {
  ...
  const related = project
    ? portfolioProjects.filter((item) => item.slug !== slug && (item.category === project.category || item.tags.includes(project.category))).slice(0, 3)
    : [];

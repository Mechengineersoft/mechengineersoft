...
export default function TestimonialsSection() {
  ...
  const storedTestimonials = usePublicContent('testimonials', []);
  const displayedTestimonials = storedTestimonials.length > 0 ? storedTestimonials : placeholders;
    ...
    <section ref={sectionRef} className="py-20 px-6 border-t border-border relative overflow-hidden" aria-labelledby="testimonials-heading">
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="grid md:grid-cols-3 gap-5 mt-12">
          {displayedTestimonials.map((item: { name: string; company: string; role?: string | null; quote: string; photo?: string | null }, index) => (
            <article
              key={index}
            ...
            >
              ...
              <div className="flex items-center gap-3 mt-7 pt-6 border-t border-border">
                {item.photo ? <img src={item.photo} alt="" className="w-11 h-11 rounded-full border border-border object-cover" /> : <span className="w-11 h-11 rounded-full border border-border bg-secondary flex items-center justify-center text-xs text-muted-foreground" aria-hidden="true">MES</span>}
                <div>

...
export type AboutPageContent = {
  ...
  visionBody?: string;
  timeline?: Array<{ year: string; tag: string; title: string; body: string; accent: string; icon?: string }>;
  coreValues?: Array<{ label: string; icon: string; color: string }>;
  education?: Array<{ degree: string; field: string; spec?: string; institution: string; year: string; icon: string; color: string }>;
  skillGroups?: Array<{ category: string; color: string; skills: string[] }>;
  differentiators?: Array<{ icon: string; title: string; body: string; accent: string }>;
};
...
export const aboutPages = pgTable("about_pages", {
  id: uuid("id").defaultRandom().primaryKey(),
  slug: varchar("slug", { length: 80 }).notNull().unique().default("about"),
  content: jsonb("content").$type<AboutPageContent>().notNull().default({}),
  status: contentStatus("status").notNull().default("published"),
  createdAt,
  updatedAt,
});

export const contentSeedState = pgTable("content_seed_state", {
  id: uuid("id").defaultRandom().primaryKey(),
  collection: varchar("collection", { length: 80 }).notNull().unique(),
...
export const portfolioItems = pgTable("portfolio_items", {
  ...
  metadata: jsonb("metadata").$type<PortfolioMetadata>().notNull().default({}),
